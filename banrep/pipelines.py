...
class BanrepPipeline:

    # Database setup
    conn = sqlite3.connect('banrep.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `banrep`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM banrep WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO banrep (date, title, link, content)"
